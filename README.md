...
### Работа с сетью
Для приема и передачи данных по сети используются соответствующие примитивы библиотеки boost::asio. Сервис является акцептором. Допускается переподключение инициатора (ядро системы). При этом одновременно несколько инициаторов быть подключено не может.
